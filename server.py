...
def post_enclaves():
    ...
    if ftype == 0 and ftype == 1 and ftype == 2:
        ...
        if verifier.verify(digest, postsig):
            post_obj = {
                'sigblock': f"{presig}::{postsig}::{token_bytes(20)}",
                'body': enc
            }
            ...
            if verify_signature(usr_id, enclave.oldsign, enclave.signedold):  # check user key
                if verify_signature(usr_id, enclave.nextsign, signed_data):  # check nextkey
                    if request.method == 'PATCH' and request.form['enclave'] is not None:
                        attribute_updates = {
                            body: {'Value': request.form['enclave'], 'Action': 'PUT'},
                            sigblock: {'Value': f"{enclave.nextsign}::{signed_data}::{token_bytes(20)}", 'Action': 'PUT'},
                        }
                        enclaveTable.update_item(
                            Key=key, AttributeUpdates=attribute_updates)
...
def post_shards():
    ...
    if verify_signature(usr_id, presig, postsig):
        ...
        post_obj = { 
            'id': ecid,
            'sigblock': f"{presig}::{postsig}::{token_bytes(20)}",
            'id_attach': request.form['id_attach'],
            'body': request.form['body']
        }
        
        if 'burn_days' in request.form and is_int(request.form['burn_days']):
            post_obj['burn_at'] = (
                datetime.now() + datetime.timedelta(days=request.form['burn_days'])).isoformat()
...
def get_shards():
   ...
   shards = shardTable.query(KeyConditionExpression=Key('id_attach').eq(id) & Key('burn_at').gte(lowbound) & Key('burn_at').lte(hibound)).get('Items')
            ...
            if verify_signature(usr_id, shard.oldsign, shard.signedold): #check user key
                if verify_signature(usr_id, shard.nextsign, signed_data): #check nextkey
                    if request.method == 'PATCH' and request.form['shard'] is not None:
                        attribute_updates = {
                            body: { 'Value': request.form['shard'], 'Action': 'PUT' },
                            sigblock: { 'Value': f"{shard.nextsign}::{signed_data}::{token_bytes(20)}", 'Action': 'PUT' }
                        }
...
def verify_signature(usr_id, presig, postsig):
    ...
    usrkey = enclaveTable.get_item(
        Key={'id': usr_id, 'type': 2}).get('Item').enclave
    digest = SHA256.new()
